...
class Ship extends Widget {
  ...
  render() {
    ...
    const shipName = decodeURIComponent(ship.get('id').split('@')[1]);
    ...
    if (posId && !destId) {
      ...
      if (celest && celest.get('type') === 'moon') {
        const planet = decodeURIComponent(celest.get('planetId').split('@')[1]);
    ...
    return (
      <div className={this.styles.classNames.ship}>
        ...
        <div className={this.styles.classNames.goods}>
          ...
          <div>Total goods : {score?.get('total') || 0}</div>
        </div>
      </div>
    );
  }
...
class Score extends Widget {
  ...
  render() {
    ...
    const shipName = decodeURIComponent(shipId.split('@')[1]);
