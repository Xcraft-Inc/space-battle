...
describe('goblin.space-battle.ship', function () {
  ...
  it('goto', function () {
    ...
    expect(shipLogic.state.server).to.be.false;
  });
  ...
  it('update', function () {
    ...
    shipLogic.goto('celestial@moon');
    shipLogic.update(200, 274, {'He-3': 666}, {});

    expect(shipLogic.state.server).to.be.true;
    expect(shipLogic.state.goods['He-3']).to.be.equal(666 - 50 * 1.5); // 591
    expect(shipLogic.state.delta).to.be.equal(50);
    expect(shipLogic.state.position).to.be.equal('celestial@earth');

    shipLogic.update(200, 274, {'He-3': 591}, {});
