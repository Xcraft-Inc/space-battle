...
class ShipLogic extends Elf.Archetype {
  ...
  reset() {
    const {fuelInitial} = battleConfig.ship;
    const {state} = this;
  ...
  beforePersistOnServer(
    ...
    const {fuelFactor, distanceMin, distanceMax} = battleConfig.ship;
    const {state} = this;
    state.server = 0;
    ...
    if (state.destination && dstDistance) {
      ...
      if (totalDistance < distanceMin) {
        totalDistance = distanceMin; /* Moon's case */
      }

      let delta = totalDistance - state.delta;
      const progress = delta > distanceMax ? distanceMax : delta;

      /* Compute fuel */
      let fuel = shipGoods['He-3'] - progress * fuelFactor;
      if (fuel < 0) {
        fuel = 0;
      }
