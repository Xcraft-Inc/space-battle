...
const {record, number, boolean, option, array} = require('xcraft-core-stones');
...
class Ship extends Elf {
  ...
  async collect(elements) {
    this.logic.collect(elements);
    await this.persist();
  }
  ...
  async beforePersistOnServer() {
    ...
    if (collect.length) {
      this.log.dbg(
        `[${this.state.id}] COLLECT ${collect.toJS().join(',')} on ${
          this.state.position
        } fuel:${this.state.goods['He-3']}`
      );

      const celestial = await new Celestial(this).create(old.position);
      const _goods = {};
      for (const element of collect) {
        /* Max 1000 units by collect call */
        _goods[element] = 1000;
      }

      extracted = await celestial.collect(_goods);
    }
    ...
    if (src && dst) {
      this.log.dbg(
        `[${this.state.id}] GOTO ${this.state.destination} fuel:${
