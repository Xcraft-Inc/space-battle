...
const CRON_RESET = '30 2/3 * * * *'; // 3min30 [2:30, 5:30, ...]
const CRON_START = '0 */3 * * * *'; //  3min   [3:00, 6:00, ...]
const CRON_TICKS = '*/2 * * * * *'; //  2s     [0:02, 0:04, ...]
...
class Universe extends Elf.Alone {
  ...
  async boot() {
    ...
    await this._cron.upsert('reset', CRON_RESET, 'universe.reset');
    await this._cron.upsert('start', CRON_START, 'universe.start');
    await this._cron.upsert('tick', CRON_TICKS, 'universe.tick');
