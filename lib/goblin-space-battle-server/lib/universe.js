...
class Universe extends Elf.Alone {
  ...
  async boot() {
    ...
    this.quest.sub(
      '*::universe.started',
      async () => await this.cron.start('tick')
    );

    this.cron = new Chronomancer(this);

    await this.cron.upsert('reset', '30 3/4 * * * *', 'universe.reset'); // 4min
    await this.cron.upsert('start', '0 */4 * * * *', 'universe.start'); // 4min 30s
    await this.cron.upsert('tick', '*/2 * * * * *', 'universe.tick'); // 2s

    await this.cron.start('start');
    await this.cron.start('reset');
  }
