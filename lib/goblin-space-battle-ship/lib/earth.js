...
const {ShipLogic, ShipState} = require('goblin-space-battle/lib/ship.js');
...
class Earth extends Elf.Alone {
  ...
  async boot() {
    ...
    this.quest.sub('*::universe.started', async () => {
      ...
      const shipdId = `ship@${pilot.toLowerCase()}`;

      this._unsub = this.quest.sub(
        `*::warehouse.<${this._desktopId}>.changed`,
        async () => {
          const data = await this.quest.warehouse.get({path: shipdId});
