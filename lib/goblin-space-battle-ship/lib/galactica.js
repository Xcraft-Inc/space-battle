...
class Galactica extends Elf.Alone {
  ...
  async boot() {
    ...
    this.quest.sub('*::universe.started', async () => await this.start());
  }

  async start() {
    const ship = await new Ship(this).create(`ship@schroeter`, this._desktopId);
    await ship.goto('celestial@mars');
  }
